...
class PipelineBasicSearchCV(BaseEstimator):
    ...
    def _combine_runs_splits(self, results_single_split,):
        ...
        if self.combine_runs:
            ...
            results_single_split = [
                ...
                for info in [
                    {'labels': labels_test, 'predictions': predictions_test, 'tort': 'test'},
                    {'labels': labels_train, 'predictions': predictions_train, 'tort': 'train'},
                    ]
                ]
            ...
            if len(self.metrics_probability) > 0:
                ...
                results_single_split.extend(
                    [
                        ...
                        for info in [
                            {'labels': labels_test, 'probabilities': probabilities_test, 'tort': 'test'},
                            {'labels': labels_train, 'probabilities': probabilities_train, 'tort': 'train'},
                            ]
                        ]
                    )
            ...
            results_single_split = [
                ...
                for nr in range(self.repeat)
                for info in [
                    {'labels': labels_test[nr], 'predictions': predictions_test[nr], 'tort': 'test'},
                    {'labels': labels_train[nr], 'predictions': predictions_train[nr], 'tort': 'train'},
                    ]
                ]
            ...
            if len(self.metrics_probability) > 0:
                ...
                results_single_split.extend(
                    [
                        ...
                        for nr in range(self.repeat)
                        for info in [
                            {'labels': labels_test[nr], 'probabilities': probabilities_test[nr], 'tort': 'test'},
                            {'labels': labels_train[nr], 'probabilities': probabilities_train[nr], 'tort': 'train'},
                            ]
                        ])
    ...
    def _test_pipeline(self, X, y, pipeline):
        ...
        def _test_pipeline_parallel(
            train_data, test_data, y, ns, nr,
            pipeline, metrics, metrics_probability, combine_splits,
            ):
    ...
    def fit(self,
        ...
        if self.cv is None:
            ...
            X = [(
                    {split_data:X[split_data][train_idx] for split_data in self.split_transform_on},
                    {split_data:X[split_data][test_idx] for split_data in self.split_transform_on}
        ...
        for pipeline_name in self.pipeline_names:
            results_pipeline = self._param_test_pipeline(
                X=X, y=y, pipeline_name=pipeline_name,
                )
        ...
        results = results[[
            'pipeline', 'repeat_number', 'split_number', 'train_or_test',
            'metric', 'value', 'splitter', 'params', 'train_id', 'param_updates',
            ]]
...
class PipelineSearchCV(PipelineBasicSearchCV):
    def __init__(
        ...
        **kwargs
        ...
        self.param_grid = {pipeline_name: [None] for pipeline_name in self.pipeline_names}
    ...
    def _param_test_pipeline(self,
        ...
        for g in self.param_grid[pipeline_name]:
            pipeline = pipeline_constructor(
                pipeline_name, name_to_object=self.name_to_object,
                verbose=False,
                )
            results_temp = {
                'pipeline': pipeline_name,
...
class PipelineBayesSearchCV(PipelineBasicSearchCV):
    ...
    def _param_test_pipeline(
        ...
        def to_optimise(**params_update):
            ...
            results_pipeline.append(results_temp_df,)
